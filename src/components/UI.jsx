...
export function KpiCard({ icon, title, value, sub, accent = 'var(--accent)', delay = 0 }) {
  return (
    <div className="card fade-in flex-1 min-w-[140px] group hover:scale-[1.01] transition-transform" style={{ animationDelay: `${delay}s` }}>
      <div className="flex items-center gap-1.5 text-[10px] text-empire-muted uppercase tracking-wider mb-2 font-semibold">
...
export function EmptyState({ message, icon = '📭' }) {
  return (
    <div className="text-center py-10">
      ...
      <p className="text-empire-muted text-xs">{message}</p>
    </div>
  );
}
...
export function AlertBanner({ type = 'warning', children }) {
  const styles = {
    warning: 'border-amber-400/30 bg-amber-500/5 text-amber-500 dark:text-amber-400',
    danger: 'border-red-400/30 bg-red-500/5 text-red-500 dark:text-red-400',
    success: 'border-emerald-400/30 bg-emerald-500/5 text-emerald-600 dark:text-emerald-400',
  };
  return (
    <div className={`card border-[1.5px] ${styles[type]} text-xs font-semibold leading-relaxed`}>
      {children}
    </div>
  );
}
...
export function FormField({ label, children }) {
  return (
    <div className="flex-1 min-w-[130px]">
      <label className="block text-[10px] text-empire-muted mb-1.5 uppercase tracking-widest font-semibold">{label}</label>
      {children}
    </div>
  );
}
...
export function Badge({ children, color = 'var(--accent)' }) {
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold" style={{ background: `${color}15`, color }}>
      {children}
    </span>
  );
}
